        ...
        <div class="col-lg-6">
            <div class="card card-custom border-0 shadow-sm">
                <div class="card-header bg-white border-0 py-3">
                    <h6 class="mb-0 fw-bold text-dark"><i class="bi bi-tags-fill me-2 text-primary-custom"></i>Form Tambah Kategori</h6>
                </div>
                                ...
                                @if($umkms->count() > 0)
                                    @foreach($umkms as $umkm)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" name="umkm_ids[]" value="{{ $umkm->id }}" id="umkm_{{ $umkm->id }}" {{ (is_array(old('umkm_ids')) && in_array($umkm->id, old('umkm_ids'))) ? 'checked' : '' }}>
